...
def log_prior(image, kernel_params, diff_stdev):
    ...
    first_order_distr = dual_normal_mixture(flat_probability, 0.02, diff_stdev)
    first_order_prior = \
        tf.math.reduce_mean(first_order_distr.log_prob(dy)) + \
        tf.math.reduce_mean(first_order_distr.log_prob(dx))
    ...
    kernel_size_prior = tfd.Gamma(2., .01).log_prob(tf.reduce_sum(tf.square(kernel_params)))
